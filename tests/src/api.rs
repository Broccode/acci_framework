...
//!
//! This module contains tests for the API layer.
...
pub mod auth_handler_test {
    ...
    use acci_auth::{
        ...
        services::user::{LoginResult, UserServiceError},
    };
    use axum::{
        Json,
        extract::State,
    ...
    fn test_successful_registration() {
        with_runtime(|rt| {
            ...
            // Act
            let response =
                rt.block_on(async { test_api_register(&state, registration_request).await });
    ...
    fn test_token_validation_invalid_token() {
        with_runtime(|rt| {
            ...
            // Act
            let response = rt
                .block_on(async { test_validate_token(&state, "invalid-token".to_string()).await });
